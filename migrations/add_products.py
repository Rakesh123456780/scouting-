...
db_path = "c:\\Users\\Rakesh\\Anti\\scoutiq.db"
target_file = "c:\\Users\\Rakesh\\Anti\\database.py"

categories = ["Electronics", "Health", "Home", "Sports", "Toys", "Automotive", "Fashion"]
...
for i in range(start_id, start_id + num_products):
    ...
    new_products_for_db.append((i, name, cat, price, original_price, emoji, rating, reviews, score, tags, sales, margin, demand, description, brand))
    new_products_str += f'    ({i}, "{name}", "{cat}", {price}, {original_price}, "{emoji}", {rating}, {reviews}, {score}, \'{tags}\', {sales}, {margin}, "{demand}", "{description}", "{brand}"),\n'
...
if idx != -1:
    new_content = content[:idx] + new_products_str + content[idx:]
    with open(target_file, "w", encoding="utf-8") as f:
        f.write(new_content)
    print("Database updated and database.py modified successfully.")
else:
